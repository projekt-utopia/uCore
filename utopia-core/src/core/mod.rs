...
impl LibraryItem {
    pub fn new(provider: &'static str, title: String, icon: Option<String>, item: library::LibraryItemModule) -> Self {
        let iprovider = ItemProvider { title, icon, status: item.status, module: provider };
    ...
    pub fn add_provider(&mut self, provider: &'static str, title: String, icon: Option<String>, status: Vec<library::LibraryItemStatus>) {
        self.providers.insert(String::from(provider), ItemProvider { title, icon, status, module: provider });
    }
    ...
    pub fn to_frontend(&self) -> library::LibraryItemFrontend {
        library::LibraryItemFrontend {
            ...
            providers: self.providers.iter().map(|(k, v)| (k.to_owned(), (
...
impl Library {
    ...
    pub fn insert(&mut self, module: &'static str, item: library::LibraryItemModule, mod_mgr: &ModuleManager) -> Result<(), ModuleNotAvailableError> {
        ...
        self.inner.entry(item.uuid.clone())
            ...
            .or_insert(LibraryItem::new(module, modinfo.name.clone(), modinfo.icon.clone(), item));
        Ok(())
    }
